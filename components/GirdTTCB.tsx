import React, { useState } from 'react';
import { Avatar, createStyles, Grid, Space, TextInput } from '@mantine/core';
import { useModals } from '@mantine/modals';
import { Divider, Group, Text } from '@mantine/core';
import { heights } from '@mantine/core/lib/components/Button/Button.styles';

const useStyles = createStyles((theme) => ({

  icon: {
    position: 'relative',
    marginLeft: 150,
    marginTop: 10,


  },
  input: {
    height: 'auto',
    paddingTop: 18,
  },
  text: {
    marginLeft: 200,
    marginTop: 70,

  },
  title: {
    color: 'blue',
    fontSize: theme.fontSizes.xl,
    marginTop:-30,
    // zIndex: 1,
  },
  title1: {
    color: 'blue',
    fontSize: theme.fontSizes.lg,
    fontWeight: 'bold'
  },
  label: {
    position: 'absolute',
    pointerEvents: 'none',
    color: 'Blue',
    fontSize: theme.fontSizes.xs,
    paddingLeft: theme.spacing.sm,
    paddingTop: theme.spacing.sm / 2,
    zIndex: 1,
  },
 
  avata_wrapper: {

    height: "180px",
    width: "180px",

    // Static media query
    '@media (max-width: 1024px)': {
      height: "100px",
      width: "100px",
    },
    '@media (max-width: 768px)': {
      height: "120px",
      width: "120px",
    },
    '@media (max-width: 425px)': {
      height: "60px",
      width: "60px",
    },
    '@media (max-width: 375px)': {
      height: "50px",
      width: "50px",
    },
    '@media (max-width: 320px)': {
      height: "40px",
      width: "40px",
      // columns: "50%",
    },
  },
  img_avt: {
    objectFit: "cover",
    borderRadius: "70%",
    width: "100%",
    height: "100%"
  }
}));
const data = [
  {
    thongtincobanungvien: {
      name: "Nguyễn Văn A",
      email: "meo@gmail.com",
      sodienthoai: "0121212121",
      ngaysinh: '19/06/2000',
      web: 'fb.com/userName',
      diachi: "Quận Ninh Kiều",
      hinhdaidien: "data:image/jpeg;base64,"
    },
    vitriungtuyen: [
      {
        tenvitri1: "Lập trình viên",
      },
      {
        tenvitri2: "Thiết kế giao diện",
      },
    ],
    kinhnghiem: "Lập trình viên tại NT 2020-2021)",
    trinhdo: "Tốt nghiệp chuyên ngành công nghệ thông tin",
  }
]


export default function Demo() {
  const { classes } = useStyles();
  return (
    <div>
      <Grid>
        <Grid.Col span={9}>
          <Text className={classes.title}>
            {data && data.map((item) => item.thongtincobanungvien.name)}
          </Text>
          < Text className={classes.title1}>
            {data && data.map((item) => item.vitriungtuyen.map((i) => i.tenvitri1))}
          </Text> <Space h="lg" />
          {/* <Grid className='flex-container'>
          <Grid.Col span={4} className='flex-item-left'>
            <Group>
              <Divider size={'lg'}
                color={'blue'}
                sx={{
                  height: '45px'
                }}
                orientation="vertical" />
              Ngày sinh <br />
              {data && data.map((item) => item.thongtincobanungvien.ngaysinh)}
            </Group>
          </Grid.Col>
          <Grid.Col  span={4}  className='flex-item-center'>
            <Group>
              <Divider size={'lg'}
                color={'blue'}
                sx={{
                  height: '45px'
                }}
                orientation="vertical" />
              Email <br />
              {data && data.map((item) => item.thongtincobanungvien.email)}
            </Group>
          </Grid.Col>
          <Grid.Col span={4} className='flex-item-right'>
            <Group>
              <Divider size={'lg'}
                color={'blue'}
                sx={{
                  height: '45px'
                }}
                orientation="vertical" />
              Số điện thoại<br />
              {data && data.map((item) => item.thongtincobanungvien.sodienthoai)}
            </Group>
          </Grid.Col>
          <Grid.Col span={4}>
            <Group>
              <Divider size={'lg'}
                color={'blue'}
                sx={{
                  height: '45px'
                }}
                orientation="vertical" />
              Website<br />
              {data && data.map((item) => item.thongtincobanungvien.web)}
            </Group>
          </Grid.Col>
          <Grid.Col span={4}>
            <Group>
              <Divider size={'lg'}
                color={'blue'}
                sx={{
                  height: '45px'
                }}
                orientation="vertical" />Địa chỉ<br />
              {data && data.map((item) => item.thongtincobanungvien.diachi)}
            </Group>
          </Grid.Col>
        </Grid> */}



        </Grid.Col>
        <Grid.Col span={3}>
          <div className={classes.avata}>
            {data && data.map((item, index: number) => {
              return (
                // <Avatar key={index} src=  {item.thongtincobanungvien.hinhdaidien} alt="Cusc-logo" radius={120} size={180}

                <div key={index} className={classes.avata_wrapper}>
                  <img className={classes.img_avt}
                    src={item.thongtincobanungvien.hinhdaidien}

                  />
                </div>
                // />
              )
            })}
          </div>

        </Grid.Col>
      </Grid>

      <div className='flex-container'>
        <div className='flex-item-left'>
          <Group>
            <Divider size={'lg'}
              color={'blue'}
              sx={{
                height: '45px'
              }}
              orientation="vertical" />
            Ngày sinh  <br />
            {data && data.map((item) => item.thongtincobanungvien.ngaysinh)}
          </Group>
        </div>
        <div className='flex-item-center'>
          <Group>
            <Divider size={'lg'}
              color={'blue'}
              sx={{
                height: '45px'
              }}
              orientation="vertical" />
            Email <br />
            {data && data.map((item) => item.thongtincobanungvien.email)}</Group>
        </div>
        <div className='flex-item-right'>
          <Group>
            <Divider size={'lg'}
              color={'blue'}
              sx={{
                height: '45px'
              }}
              orientation="vertical" />
            Số điện thoại<br />
            {data && data.map((item) => item.thongtincobanungvien.sodienthoai)}</Group>
        </div>
      </div>
    </div>
  );
}